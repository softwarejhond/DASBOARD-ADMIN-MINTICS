...
if (isset($_POST['id']) && isset($_POST['nuevoPrograma']) && isset($_POST['nuevoNivel'])) {
    // Depuración: Verifica los valores recibidos
    error_log("ID recibido: " . $_POST['id']);
    error_log("Nuevo Programa recibido: " . $_POST['nuevoPrograma']);
    error_log("Nuevo Nivel recibido: " . $_POST['nuevoNivel']);
    ...
    // Consulta SQL para actualizar programa, nivel y fecha de actualización
    $updateSql = "UPDATE user_register SET program = ?, level = ?, dayUpdate = NOW() WHERE number_id = ?";
    $stmt = $conn->prepare($updateSql);

    if ($stmt) {
        // Vincular los parámetros para la consulta preparada
        $stmt->bind_param('ssi', $nuevoPrograma, $nuevoNivel, $id);
