...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar_documento'])) {
    ...
    if ($frontFile['error'] === UPLOAD_ERR_OK && $backFile['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($frontFile['tmp_name'], $frontPath) && move_uploaded_file($backFile['tmp_name'], $backPath)) {
            ...
            if ($stmt->execute()) {
                echo "<script>alert('Documentos actualizados correctamente.');</script>";
            } else {
                echo "<script>alert('Error al actualizar los documentos.');</script>";
            }
        } else {
            echo "<script>alert('Error al subir los archivos.');</script>";
        }
    } else {
        echo "<script>alert('Debes seleccionar ambos archivos.');</script>";
    }
}
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                return fetch(window.location.href, {
                    ...
                    .then(html => {
                        if (html.includes('correctamente')) {
